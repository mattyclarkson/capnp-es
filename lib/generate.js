...
import parse from './pointer/parse.js';
...
export default function(data, logger=log) {
  if (!(data instanceof DataView)) {
    throw new TypeError('Must provide a valid DataView');
  }
  ...
  */
  ...
  function* size() {
    ...
    for (let i = 1; i <= segments; ++i) {
      const words = data.getUint32(4 * i, true);
      yield new DataView(data.buffer, byteOffset, (words * 8));
    }
  }
  ...
  console.log(parse(new DataView(segment.buffer, segment.byteOffset, 8)));
